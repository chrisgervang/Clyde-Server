...
var checkConditions = function(config, device, cb) {
	...
	if (device.designRef === "weather") {
		...
		if (config.func === 'dateDayOfWeek') {
			var setDateDay = _.find(config.data, {dataType: "setDateDay"});
			//TODO: load in user's locale
			var us = tz(require("timezone/America"));
			console.log("DAY OF WEEK", us(device.state.timeStamp, "%u"));
			if(setDateDay.dataValue === "monday" && us(device.state.timeStamp, "%u") === 1) {
				cb(true);
			} else if(setDateDay.dataValue === "tuesday" && us(device.state.timeStamp, "%u") === 2) {
				cb(true);
			} else if(setDateDay.dataValue === "wednesday" && us(device.state.timeStamp, "%u") === 3) {
				cb(true);
			} else if(setDateDay.dataValue === "thursday" && us(device.state.timeStamp, "%u") === 4) {
				cb(true);
			} else if(setDateDay.dataValue === "friday" && us(device.state.timeStamp, "%u") === 5) {
				cb(true);
			} else if(setDateDay.dataValue === "saterday" && us(device.state.timeStamp, "%u") === 6) {
				cb(true);
			} else if(setDateDay.dataValue === "sunday" && us(device.state.timeStamp, "%u") === 7) {
				cb(true);
			} else if(setDateDay.dataValue === "weekday" && us(device.state.timeStamp, "%u") <= 5 && us(device.state.timeStamp, "%u") >= 1) {
				cb(true);
			} else if(setDateDay.dataValue === "weekend" && us(device.state.timeStamp, "%u") >= 6 && us(device.state.timeStamp, "%u") <= 7) {
				cb(true);
			} else {
				cb(false);
			}
		...
		if (config.func === 'timeSunrise') {
			...
			if(setIfBeforeAfter.dataValue === 'before') {
				...
				var beforeSunriseTime = device.state.sunrise - (setTimeMedium.dataValue * 60000);
				...
				var afterSunriseTime = device.state.sunrise + (setTimeMedium.dataValue * 60000);
			...
			if(setIfBeforeAfter.dataValue === 'before') {
				...
				var beforeSunsetTime = device.state.sunset - (setTimeMedium.dataValue * 60000);
				...
				var afterSunsetTime = device.state.sunrise + (setTimeMedium.dataValue * 60000);
			...
			//console.log("BOAT", tz(settime24hour.dataValue, "%H"), tz(settime24hour.dataValue, "%M"));
			...
			data.settings.process = setInterval(function(){
				that.ask(function(result) {
					...
					if (tz(data.state.timeStamp, "%M") !== tz(result.timeStamp, "%M")) {
						...
						new Firebase("https://clydedev.firebaseio.com/devices/" + data.id + "/state/").update({timeStamp: data.state.timeStamp})
					...
					if(!!result.sunrise && data.state.sunrise !== result.sunrise) {
						...
						new Firebase("https://clydedev.firebaseio.com/devices/" + data.id + "/state/").update({sunrise: data.state.sunrise})
					}
					...
					if(!!result.sunset && data.state.sunset !== result.sunset) {
						...
						new Firebase("https://clydedev.firebaseio.com/devices/" + data.id + "/state/").update({sunset: data.state.sunset})
					}
				});
